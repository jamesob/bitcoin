    ...
    if (sigversion == SigVersion::TAPSCRIPT) {
        ...
        while (pc < exec_script.end()) {
            ...
            if (IsOpSuccess(opcode)) {
                if (opcode == OP_VAULT || opcode == OP_VAULT_RECOVER) {
                    if (flags & SCRIPT_VERIFY_DISCOURAGE_VAULT) {
                        return set_error(serror, SCRIPT_ERR_BAD_OPCODE);
                    } else if ((flags & SCRIPT_VERIFY_VAULT) == 0) {
                        // retain OP_SUCCESS behaviour if soft-fork hasn't activated
                        return set_success(serror);
                    }
                } else {
