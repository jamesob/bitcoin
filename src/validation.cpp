...
std::shared_ptr<CChainState> ChainstateActive()
{
...
// Internal stuff
namespace {
    CBlockIndex *&pindexBestInvalid = g_blockmetaman.pindexBestInvalid;
...
bool CheckFinalTx(const CTransaction &tx, int flags)
{
    ...
    // CheckFinalTx() uses chain.Height()+1 to evaluate
    // nLockTime because when IsFinalTx() is called within
    // CBlock::AcceptBlock(), the height of the block *being*
    // evaluated is what is used. Thus if we want to know if a
    // transaction can be part of the *next* block, we need to call
    // IsFinalTx() with one more than chain.Height().
    const int nBlockHeight = g_chainman.ActiveHeight() + 1;
...
bool CheckSequenceLocks(const CTxMemPool& pool, const CTransaction& tx, int flags, LockPoints* lp, bool useExistingLockPoints)
{
    ...
    CBlockIndex index;
    index.pprev = tip;
    // CheckSequenceLocks() uses chain.Height()+1 to evaluate
    // height based locks because when SequenceLocks() is called within
    // ConnectBlock(), the height of the block *being*
    // evaluated is what is used.
    // Thus if we want to know if a transaction can be part of the
    // *next* block, we need to use one more than chain.Height()
...
static void LimitMempoolSize(CTxMemPool& pool, size_t limit, unsigned long age) {
    ...
    for (const COutPoint& removed : vNoSpendsRemaining)
        g_chainman.ActiveCoinsCache()->Uncache(removed);
}
...
static void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    mempool.removeForReorg(g_chainman.ActiveCoinsCache(), g_chainman.ActiveTip()->nHeight + 1, STANDARD_LOCKTIME_VERIFY_FLAGS);
 ...
 *
 * @param[in] primary_view  A coins view that is not based on the mempool.
 */
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool,
                 unsigned int flags, bool cacheSigStore, PrecomputedTransactionData& txdata,
                 CCoinsViewCache* primary_view) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    AssertLockHeld(cs_main);
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (txFrom) {
            ...
            // We explicitly avoid using the `view` Cache here so that we know
            // we aren't using a mempool-based coins view.
            const Coin& coinFromDisk = primary_view->AccessCoin(txin.prevout);
...
static bool AcceptToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx,
                              ...
                              bool test_accept, CCoinsViewCache* primary_coins_view) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        LockPoints lp;
        CCoinsViewMemPool viewMemPool(primary_coins_view, pool);
        view.SetBackend(viewMemPool);

        // do all inputs exist?
        for (const CTxIn& txin : tx.vin) {
            if (!primary_coins_view->HaveCoinInCache(txin.prevout)) {
            ...
            if (!view.HaveCoin(txin.prevout)) {
                ...
                for (size_t out = 0; out < tx.vout.size(); out++) {
                    ...
                    if (primary_coins_view->HaveCoinInCache(COutPoint(hash, out))) {
        ...
        unsigned int currentBlockScriptVerifyFlags = GetBlockScriptFlags(
            g_chainman.ActiveTip(), chainparams.GetConsensus());

        if (!CheckInputsFromMempoolAndCache(
                    tx, state, view, pool, currentBlockScriptVerifyFlags, true, txdata, primary_coins_view)) {
                        ...
                        bool bypass_limits, const CAmount nAbsurdFee, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    CCoinsViewCache* primary_coins_view = g_chainman.ActiveCoinsCache();
    bool res = AcceptToMemoryPoolWorker(
            ...
            /* primary_coins_view= */ primary_coins_view);
    if (!res) {
        for (const COutPoint& hashTx : coins_to_uncache)
            primary_coins_view->Uncache(hashTx);
    }
...
// Note that though this is marked const, we may end up modifying `m_is_ibd`, which
...
bool CChainState::IsInitialBlockDownload() const
{
    ...
    if (m_is_ibd.load(std::memory_order_relaxed))
        return false;
    ...
    m_is_ibd.store(true, std::memory_order_relaxed);
    return false;
}
...
static void CheckForkWarningConditions() EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // of our head, drop it
    if (pindexBestForkTip && g_chainman.ActiveHeight() - pindexBestForkTip->nHeight >= 72)
        pindexBestForkTip = nullptr;

    CBlockIndex* tip = g_chainman.ActiveTip();

    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > tip->nChainWork + (GetBlockProof(*tip) * 6)))
    {
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    CBlockIndex* plonger = g_chainman.ActiveTip();
    ...
    if (pfork && (!pindexBestForkTip || pindexNewForkTip->nHeight > pindexBestForkTip->nHeight) &&
            ...
            g_chainman.ActiveHeight() - pindexNewForkTip->nHeight < 72)
    {
...
void static InvalidChainFound(CBlockIndex* pindexNew) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    LogPrintf("%s:  current best=%s  height=%d  log2_work=%.8g  date=%s\n", __func__,
      tip->GetBlockHash().ToString(), g_chainman.ActiveHeight(), log(tip->nChainWork.getdouble())/log(2.0),
...
void FlushStateToDisk(CChainState* chainstate) {
    if (!chainstate) {
        chainstate = ::ChainstateActive().get();
    }
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams, const CCoinsViewCache* coins_view) {
    ...
    // TODO jamesob: maybe parameterize this for the background IBD? For now
    // just ignore.
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__, /* Continued */
      ...
      primary_coins_view->DynamicMemoryUsage() * (1.0 / (1<<20)),
      primary_coins_view->GetCacheSize());
...
bool CChainState::DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    {
        CCoinsViewCache view(coins_cache);
    ...
    UpdateTip(pindexDelete->pprev, chainparams, m_coins_views->m_cacheview.get());
...
/**
 * Connect a new block to the chain. pblock is either nullptr or a pointer to a CBlock
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    if (this == ::ChainstateActive().get()) {
        mempool.removeForBlock(blockConnecting.vtx, pindexNew->nHeight);
        disconnectpool.removeForBlock(blockConnecting.vtx);
    }

    m_chain.SetTip(pindexNew);
    UpdateTip(pindexNew, chainparams, coins_cache);
            ...
            if (fFailedChain || fMissingData) {
                ...
                while (pindexTest != pindexFailed) {
                    if (fFailedChain) {
                        ...
                        g_blockmetaman.m_blocks_unlinked.insert(std::make_pair(pindexFailed->pprev, pindexFailed));
                    }
...
bool CChainState::ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    mempool.check(m_coins_views->m_cacheview.get());
...
void CChainState::Delete()
{
    ...
    LOCK(cs_main);
    m_coins_views->m_dbview->MarkForDeletion();

    // on-disk coinsdb data is deleted during destruction here.
    ResetCoinsViews();

    for (std::shared_ptr<CChainState> cstate : g_chainman.GetAll()) {
        if (cstate.get() == this) {
            cstate.reset();
        }
    }
        ...
        while (!queue.empty()) {
            ...
            std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator,
                std::multimap<CBlockIndex*, CBlockIndex*>::iterator> range =
                    g_blockmetaman.m_blocks_unlinked.equal_range(pindex);
...
bool BlockMetadataManager::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!pindexPrev->IsValid(BLOCK_VALID_SCRIPTS)) {
            ...
            for (const CBlockIndex* failedit : g_blockmetaman.m_failed_blocks) {
    ...
    g_chainman.RunOnAll([&chainparams](std::shared_ptr<CChainState> chainstate) {
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
    ...
    {
        LOCK(cs_main);
        chainstate = g_chainman.GetChainstateForNewBlock(pblock->GetHash()).get();
    }
...
{
    AssertLockHeld(cs_main);
    assert(pindexPrev && pindexPrev == g_chainman.ActiveTip());
    CCoinsViewCache viewNew(g_chainman.ActiveCoinsCache());
...
static void FindFilesToPruneManual(std::set<int>& setFilesToPrune, int nManualPruneHeight)
{
    ...
    for (std::shared_ptr<CChainState> chainstate : g_chainman.GetAll()) {
        ...
        unsigned int start_prune_height = 0;
        bool is_snapshot_chain_while_validating = (
            g_chainman.m_ibd_chainstate &&
            chainstate == g_chainman.m_snapshot_chainstate);
        ...
        //
        if (g_chainman.IsBackgroundValidationChainstate(chainstate.get())) {
            nLastBlockWeCanPrune = std::min(
                nManualPruneHeight,
                // TODO jamesob: is this an off-by-one?
                chainstate->m_chain.Height() - 1);
        }
        // Otherwise, if we're pruning the snapshot chainstate, be sure not to
        // step on the toes of the background validation by pruning blocks it
        // might be currently using.
        //
        else if (is_snapshot_chain_while_validating) {
            start_prune_height = g_chainman.m_ibd_chainstate->m_chain.Height();
        }
        ...
        for (int fileNumber = 0; fileNumber < nLastBlockFile; fileNumber++) {
            if (vinfoBlockFile[fileNumber].nSize == 0 ||
                    ...
                    vinfoBlockFile[fileNumber].nHeightFirst <= start_prune_height)
                continue;
    ...
    for (std::shared_ptr<CChainState> chainstate : g_chainman.GetAll()) {
        ...
        if (nCurrentUsage + nBuffer >= nPruneTarget) {
            ...
            for (int fileNumber = 0; fileNumber < nLastBlockFile; fileNumber++) {
                ...
                if (vinfoBlockFile[fileNumber].nHeightFirst <= start_prune_height) {
                    continue;
                }
...
bool BlockMetadataManager::LoadBlockIndex(const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS) && (pindex->HaveTxsDownloaded() || pindex->pprev == nullptr))
            // TODO jamesob: is this what we want? Special handling for the snapshot chain?
            g_chainman.RunOnAll([pindex](std::shared_ptr<CChainState> chainstate) {
...
void CChainState::EraseBlockData(CBlockIndex* index)
{
    ...
    std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator, std::multimap<CBlockIndex*, CBlockIndex*>::iterator> ret = g_blockmetaman.m_blocks_unlinked.equal_range(index->pprev);
                ...
                uint256 hash = block.GetHash();
                {
                    ...
                    if (!pindex || (pindex->nStatus & BLOCK_HAVE_DATA) == 0) {
                      ...
                      if (::ChainstateActive()->AcceptBlock(
                            pblock, state, chainparams, nullptr, true, dbp, nullptr)) {
                          nLoaded++;
                      }
                      if (state.IsError()) {
                          break;
...
{
    LOCK(cs_main);
    return VersionBitsState(g_chainman.ActiveTip(), params, pos, versionbitscache);
}

BIP9Stats VersionBitsTipStatistics(const Consensus::Params& params, Consensus::DeploymentPos pos)
{
    LOCK(cs_main);
    return VersionBitsStatistics(g_chainman.ActiveTip(), params, pos);
}

int VersionBitsTipStateSinceHeight(const Consensus::Params& params, Consensus::DeploymentPos pos)
{
    LOCK(cs_main);
    return VersionBitsStateSinceHeight(g_chainman.ActiveTip(), params, pos, versionbitscache);
}
...
std::vector<std::shared_ptr<CChainState>> ChainstateManager::GetAll() const
{
    std::vector<std::shared_ptr<CChainState>> out;

    if (!IsSnapshotValidated() && m_ibd_chainstate) {
        out.push_back(m_ibd_chainstate);
    }

    if (m_snapshot_chainstate) {
        out.push_back(m_snapshot_chainstate);
    }
...
void ChainstateManager::RunOnAll(
    const std::function<void(std::shared_ptr<CChainState>)> fn) const
{
    for (std::shared_ptr<CChainState> chainstate : GetAll()) {
        fn(chainstate);
    }
}

std::vector<std::shared_ptr<CChainState>> ChainstateManager::GetAllForBlockDownload() const
{
    std::vector<std::shared_ptr<CChainState>> out;

    if (m_snapshot_chainstate) out.push_back(m_snapshot_chainstate);
    if (!IsSnapshotValidated() && m_ibd_chainstate) out.push_back(m_ibd_chainstate);

    return out;
}

std::shared_ptr<CChainState>& ChainstateManager::InitializeChainstate(
        ...
        bool snapshot_needs_initialization)
{
    ...
    std::shared_ptr<CChainState>& to_modify = (
    ...
    if (activate) {
        ...
        m_active_chainstate = to_modify;
    }

    return to_modify;
}

std::string ChainstateManager::ToString()
{
    std::string out;
    out += tfm::format("m_ibd: %p\n", m_ibd_chainstate.get());
    out += tfm::format("m_snapshot: %p\n", m_snapshot_chainstate.get());
    out += tfm::format("m_active: %p\n", m_active_chainstate.get());
    return out;
}
        ...
        bool in_memory)
{
    ...
    std::shared_ptr<CChainState>& snapshot_chainstate = InitializeChainstate(
    ...
    m_active_chainstate = m_snapshot_chainstate;
    m_snapshot_metadata = MakeUnique<SnapshotMetadata>(metadata);
    ...
    CChainState* validation_chainstate)
{
    ...
    if (snapshot_invalid) {
        ...
        m_active_chainstate = m_ibd_chainstate;
        m_snapshot_chainstate->MarkForDeletion();
    ...
    SaveSnapshotMetadataToDisk();
    m_ibd_chainstate->MarkForDeletion();

    return true;
}

std::shared_ptr<CChainState> ChainstateManager::GetChainstateForNewBlock(const uint256& blockhash)
{
    if (m_snapshot_chainstate &&
...
{
    auto has_tip = [](std::shared_ptr<CChainState> cs) {
        return cs && cs->m_chain.Tip();
    };
    if (has_tip(m_ibd_chainstate) && has_tip(m_snapshot_chainstate))
        assert(m_ibd_chainstate->m_chain.Tip() != m_snapshot_chainstate->m_chain.Tip());
