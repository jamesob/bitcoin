...
void PruneAndFlush(CChainState* chainstate = nullptr);
...
public:
    ...
    std::unique_ptr<CoinsViewErrorCatcher> m_catcherview;
            ...
            bool should_wipe = false)
    {
        ...
        m_catcherview.reset(new CoinsViewErrorCatcher(m_dbview.get()));
...
public:
                ...
                m_coins_cache_size_bytes(cache_size_bytes)
    {
        ...
        m_coins_views.reset(new CoinsViews(leveldb_name, cache_size_bytes, in_memory, should_wipe));
    }
    ...
    CCoinsViewCache* CoinsCache() const { return m_coins_views->m_cacheview.get(); }

    void ResetCoinsViews() { m_coins_views.reset(); }

    void MarkForDeletion() { m_should_delete = true; }
    ...
    // Block disconnection on our tip:
    ...
    /**
     * When there are blocks in the active chain with missing data, rewind the chainstate and
     * remove them from the block index.
     */
...
private:
     ...
     */
    std::shared_ptr<CChainState> m_unready_snapshot_chainstate;
...
public:
     ...
     */
    std::shared_ptr<CChainState> m_ibd_chainstate;

    /**
     * A chainstate initialized on the basis of a UTXO snapshot. If this is
     * truthy, it is always our active chainstate unless proven invalid.
     */
    std::shared_ptr<CChainState> m_snapshot_chainstate;

    /**
     * Points to either the ibd or snapshot chainstate; indicates our
     * most-work chain.
     */
    std::shared_ptr<CChainState> m_active_chainstate;

    ChainstateManager() { m_active_chainstate = m_ibd_chainstate; }
     ...
     */
    std::shared_ptr<CChainState>& InitializeChainstate(
        size_t cache_size_bytes,
        bool in_memory,
        bool should_wipe,
        bool activate = true,
    ...
    std::vector<std::shared_ptr<CChainState>> GetAll() const;

    /** Run some function on all chainstates in use.  */
    void RunOnAll(const std::function<void(std::shared_ptr<CChainState>)> fn) const;
     ...
     */
    std::vector<std::shared_ptr<CChainState>> GetAllForBlockDownload() const;
     ...
     *
     * If the coins match (expected), then destruct the validation chainstate
     * and continue using the snapshot chainstate as active. Otherwise, revert
     * to using the ibd chainstate and shutdown.
     */
     ...
     * chain.
     */
    std::shared_ptr<CChainState> GetChainstateForNewBlock(
    ...
    {
        return m_snapshot_chainstate && m_active_chainstate.get() == m_snapshot_chainstate.get();
    }
     ...
     */
    std::shared_ptr<CChainState> ValidatedChainstate() const
    {
    ...
    void Unload()
    {
        RunOnAll([](std::shared_ptr<CChainState> chainstate) {
